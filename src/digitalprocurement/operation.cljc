(ns digitalprocurement.operation
  "OperationActor -- one Digital Agency compliance operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (DigitalProcurement-LLM) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Digital
  Procurement Compliance Governor (:govern) and the rollout phase gate
  ...
  One graph run = one compliance operation (gBizID track or Gov-Cloud
  track). No unbounded inner loop.
  ...
  governor is clean -- see `digitalprocurement.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [digitalprocurement.digitalprocurementllm :as digitalprocurementllm]
            [digitalprocurement.governor :as governor]
            [digitalprocurement.phase :as phase]
...
(defn build
  ...
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (digitalprocurementllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (digitalprocurementllm/-advise advisor store request)]
            {:proposal p :audit [(digitalprocurementllm/trace request p)]})))
