...
(defn- tmpfile [suffix]
  (path/join (os/tmpdir) (str "digital-mutation-" suffix ".edn")))
...
(def laws-only
  ...
  (subset #{"law.digital-agency-establishment"}))
...
(def with-pages
  ...
  branches."
  (subset #{"host.www-digital-go-jp" "host.pr-gbiz-id-go-jp"
            "host.gbiz-id-go-jp" "law.digital-agency-establishment"
            "digital.home" "digital.about" "gbizid.about" "gbizid.top"}))
...
(def structural
  [{:id "coverage-entry-count"
   ...
   {:id "law-entry-without-law-id"
    ...
    :mutate #(alter-entity % "law.digital-agency-establishment" (fn [e] (dissoc e :egov/law-id)))
   ...
   {:id "page-on-undeclared-host"
    ...
    :mutate #(alter-entity % "digital.about"
...
(def network
  [{:id "nonexistent-law" :on :laws-only
    ...
    :mutate #(alter-entity % "law.digital-agency-establishment" (fn [e] (assoc e :egov/law-id "999ZZ9999999999")))
    :want-exit 1 :want-reason "nonexistent-law"}

   {:id "law-title-drift" :on :laws-only
    :why "a real id recorded under the wrong title"
    :mutate #(alter-entity % "law.digital-agency-establishment" (fn [e] (assoc e :egov/law-title "デジタル庁法")))
    :want-exit 1 :want-reason "law-title-drift"}

   {:id "law-num-drift" :on :laws-only
    :why "a real id recorded under the wrong law number"
    :mutate #(alter-entity % "law.digital-agency-establishment" (fn [e] (assoc e :egov/law-num "令和三年法律第三十七号")))
   ...
   {:id "fail-outranks-refusal" :on :laws-only
    ...
    :mutate (fn [d]
              (recount
                (-> d
                    (alter-entity "law.digital-agency-establishment"
                                  (fn [e] (assoc e :egov/law-title "デジタル庁法")))
                    (into UNROUTABLE-HOST))))
    :want-exit 1 :want-reason "law-title-drift"}

   {:id "page-title-drift"
    :why "a real page recorded under a title it does not carry"
    :mutate #(alter-entity % "digital.home" (fn [e] (assoc e :page/title "デジタル庁ホーム")))
    :want-exit 1 :want-reason "title-drift"}

   {:id "missing-text"
    :why "a string the page does not contain"
    :mutate #(alter-entity % "gbizid.top"
                           (fn [e] (assoc e :page/must-contain ["第9次エネルギー基本計画"])))
   ...
   {:id "charset-drift"
    ...
    :mutate #(alter-entity % "digital.home" (fn [e] (assoc e :page/charset "shift_jis")))
    :want-exit 1 :want-reason "charset-drift"}

   {:id "unexpected-redirect"
    :why "a URL that lands somewhere other than where it asked"
    ;; digital.about, not gbizid.about: the http:// form drops the entry from
    ;; the page self-test pool (which only takes https URLs), and mutating
    ;; gbizid.about would leave that pool on one host -- an honest refusal,
    ;; exit 2, before the redirect branch ever ran.
    :mutate #(alter-entity % "digital.about"
                           (fn [e] (assoc e :source/url "http://www.digital.go.jp/about")))
    :want-exit 1 :want-reason "unexpected-redirect"}

   {:id "not-2xx"
    :why "a cited page that is not there"
    ;; gbizid.top, not gbizid.about: a nonexistent URL inside the page
    ;; self-test pool (digital.about / gbizid.about) breaks the title and
    ;; charset self-tests before the not-2xx branch is even judged, and the
    ;; run refuses -- exit 2 -- for the wrong reason. The :page-text entry is
    ;; not in the pool.
    :mutate #(alter-entity % "gbizid.top"
                           (fn [e] (assoc e :source/url "https://gbiz-id.go.jp/no-such-zzz.html")))
    :want-exit 1 :want-reason "not-2xx"}

   {:id "host-status-drift"
    :why "a host whose refusal is not the refusal the register measured"
    :mutate #(alter-entity % "host.pr-gbiz-id-go-jp"
